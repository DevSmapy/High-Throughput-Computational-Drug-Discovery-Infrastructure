...
각 서버에서 생성된 CSV를 Python으로 정제했습니다. 이 과정에서 전체 컬럼이 동일한 중복 행만 제거하고, 컬럼과 데이터 타입을 정리했습니다. 같은 ZINC ID라도 속성값이 다른 행은 별도 충돌 규칙으로 병합하지 않았습니다.

정제된 결과는 SQLite에 적재하여 다음 두 계층으로 구성했습니다.

- **Compound DB:** ZINC ID를 키로 하는 화합물 레코드. SMILES, Backbone, ring count, ZINC 제공 화학 속성을 관리
...
이들은 배치 진행에 대한 운영 확인이며, 레코드 단위의 입출력 완전성·화학적 타당성·식별자 유일성 검증은 아닙니다. 이 체계는 단순하지만, 장애 상태를 한곳에 모아 판단하고 복구 작업을 계획하는 데 필요한 운영 정보를 제공했습니다.
