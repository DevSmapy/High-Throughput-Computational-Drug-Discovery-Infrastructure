...
各サーバーで生成されたCSVをPythonで整備しました。全カラムが一致する重複行のみを除去し、カラムとデータ型を整理しました。同一ZINC IDでもプロパティ値が異なる行は、別の衝突ルールでは統合していません。

整備済みデータはSQLiteにロードし、次の二つのレイヤーとして構成しました。

- **Compound DB:** ZINC IDをキーとする化合物レコード。SMILES、Backbone、ring count、ZINC提供の化学特性を管理
...
これらはバッチ進捗の運用確認であり、レコード単位の入出力完全性・化学的妥当性・識別子一意性の検証ではありません。この管理方式により、障害を一か所に集約して判断し、復旧作業を計画するための運用情報を確保しました。
